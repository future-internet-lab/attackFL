...
class Validation:
    ...
    def test_hyper(self, hnet, num_client, device):
        self.model.to(device)
        if self.data_name == "ICU":
            return self.test_hyper_icu(hnet, num_client, device)
        elif self.data_name == "CIFAR10":
            return self.test_hyper_image(hnet, num_client, device)
        else:
            raise ValueError(f"Not found test function for data name {self.data_name}")

    def test_hyper_image(self, hnet, num_client, device):
        test_loss = 0
        correct = 0
    ...
    def test_hyper_icu(self, hnet, num_client, device):
        # List to store true labels and predicted scores
        all_labels = []
        all_outputs = []
